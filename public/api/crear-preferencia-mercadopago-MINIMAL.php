<?php
// Archivo PHP mínimo para probar MercadoPago
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!$input) {
            echo json_encode([
                ...
                'debug' => 'No se pudieron decodificar los datos JSON'
            ]);
            exit();
        }
        ...
        if ($curl_error) {
            ...
            if (isset($data['init_point'])) {
                echo json_encode([
                    ...
                    'debug' => 'Preferencia creada exitosamente'
                ]);
            } else {
                echo json_encode([
                    'success' => false,
                    'message' => 'Respuesta inválida de MercadoPago',
                    'debug' => 'No se encontró init_point en la respuesta',
            ...
            echo json_encode([
                ...
                'debug' => 'Error HTTP ' . $http_code,
                'raw_response' => $response,
                'token_used' => substr($access_token, 0, 20) . '...'
            ]);
        }
    } else {
        echo json_encode([
